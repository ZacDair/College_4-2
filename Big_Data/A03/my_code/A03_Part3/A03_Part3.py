...
def my_main(spark, my_dataset_dir, year):
    ...
    data = inputDF.select("start_time", "user_type", "birth_year")
    years = data.filter("birth_year >= " + str(year))
    subscribers = years.filter("user_type == 'Subscriber'")
    startHour = subscribers.withColumn("start_hour", subscribers.start_time.substr(12, 2))
    startHour = startHour.drop("start_time")
    indexedDF = startHour.withColumn("key", pyspark.sql.functions.concat_ws("-", startHour["birth_year"], startHour["start_hour"]))
    counts = indexedDF.groupby("key").count()
    counts = counts.withColumn("birth_year", counts.key.substr(0, 4)).withColumn("start_hour", counts.key.substr(6, 2))
    maxTrips = counts.groupBy("birth_year").max("count")
    result = maxTrips.join(counts, [maxTrips["max(Count)"] == counts["count"], maxTrips["birth_year"] == counts["birth_year"]], "inner").drop(maxTrips["birth_year"])
    solutionDF = result.select("birth_year", "start_hour", "max(Count)").withColumnRenamed("max(Count)", "num_trips").drop("count", "key").sort("birth_year", ascending=True)
    solutionDF = solutionDF.withColumn("birth_year", solutionDF["birth_year"].cast(pyspark.sql.types.IntegerType()))
