...
def my_main(input_folder, output_file, year):
    ...
    for file in datasetFiles:
        ...
        for line in fileStream:
            ...
            if data[12] == "Subscriber":
                if data[13] >= year:
                    myTime = str(data[0][11:13])
                    myYear = str(data[13])
                    if (myTime + "-" + myYear) in birthYearDict:
                        birthYearDict[(myTime + "-" + myYear)] += 1
                    else:
                        birthYearDict[(myTime + "-" + myYear)] = 1
    # Sort the years
    sortedYears = {}
    for x in birthYearDict:
        values = x.split("-")
        if values[1] in sortedYears:
            if sortedYears[values[1]][1] < birthYearDict[x]:
                sortedYears[values[1]] = [values[0], birthYearDict[x]]
        else:
            sortedYears[values[1]] = [values[0], birthYearDict[x]]
    # Output
    for v in sorted(sortedYears):
        res = v + "\t" + "(" + str(sortedYears[v][0]) + "," + str(sortedYears[v][1]) + ")" + "\n"
        outFile.write(res)
    pass
