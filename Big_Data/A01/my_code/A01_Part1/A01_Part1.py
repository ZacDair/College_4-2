...
def my_main(input_folder, output_file):
    ...
    # Get all files
    datasetFiles = os.listdir(input_folder)

    # Open output file
    outFile = open(output_file, "w")

    # Initialize trip counting dict
    tripCountDict = {}

    # Loop through all files, opening them, and looping through lines
    for file in datasetFiles:
        fileStream = open(input_folder + file, "r")
        for line in fileStream:
            data = process_line(line)

            # Add start station, and increment count.   tripCountDict{station_name: [start count, stop count]}
            if data[4] not in tripCountDict:
                tripCountDict[data[4]] = [0, 0]
            tripCountDict[data[4]][0] += 1

            # Add stop station, and increment count
            if data[8] not in tripCountDict:
                tripCountDict[data[8]] = [0, 0]
            tripCountDict[data[8]][1] += 1

    # Loop through the sorted stations, creating the result string and storing in the output file
    for station in sorted(tripCountDict):
        res = station + "\t" + "(" + str(tripCountDict[station][0]) + "," + str(tripCountDict[station][1]) + ")" + "\n"
        outFile.write(res)
