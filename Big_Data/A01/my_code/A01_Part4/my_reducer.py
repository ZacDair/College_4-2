...
def processLine(line):
    ...
    station = data[0]
    counts = data[1].split(",")
    startCount = int(counts[0].replace("(", ""))
    stopCount = int(counts[1].replace(")", ""))
    return station, startCount, stopCount
...
def my_reduce(my_input_stream, my_output_stream, my_reducer_input_parameters):
    pass
    currentStation = ""
    currentStationStartCount = 0
    currentStationStopCount = 0

    for line in my_input_stream:
        station, startCount, stopCount = processLine(line)

        if station != currentStation:
            if currentStation != "":
                res = currentStation + "\t" + "(" + str(currentStationStartCount) + "," + str(currentStationStopCount) + ")" + "\n"
        ...
        currentStationStartCount += startCount
        currentStationStopCount += stopCount
