...
def my_main(input_folder, output_file, top_n_bikes):
    ...
    for file in datasetFiles:
        ...
        for line in fileStream:
            ...
            # Add bikeID, and increment count.   bikeCountDict{bikeID: [total_trip_length, trip_count]}
            if data[11] not in bikeCountDict:
                bikeCountDict[data[11]] = [0, 0]

            bikeCountDict[data[11]][0] += data[2]
            bikeCountDict[data[11]][1] += 1

    # Sort the bikes by trip duration
    orderedBikes = sorted(bikeCountDict.items(), key=getTripLength, reverse=True)

    # Output the top_n_bikes to the file
