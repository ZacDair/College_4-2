...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    bikeId = my_mapper_input_parameters[0]

    # Initialize trip counting truckTrips = [previous trip & current-start time, stop time, start station, stop station]
    truckTrips = []
    ...
    for line in my_input_stream:
        ...
        if data[11] == bikeId:
            truckTrips.extend([data[0], data[1], data[4], data[8]])

    # Convert the truck trip list into a string replacing certain symbols to match expected format
    if len(truckTrips) != 0:
        tripString = str(truckTrips)
        tripString = tripString.replace("'", "")
        tripString = tripString.replace(", ", " @ ")
        tripString = tripString.replace("[", "(")
        tripString = tripString.replace("]", ")")
        res = "universal" + "\t" + tripString + "\n"
