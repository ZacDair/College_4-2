...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        ...
        for w in words:
            # 3.2.1. We check if the word has appeared in the dictionary before. If not, we enter the word
            if (w not in my_words_dict):
                my_words_dict[w] = 0

            # 3.2.2. Update the dictionary with the new word
            my_words_dict[w] += 1

    # 4. We write the content from my_words_dict into the result file
    for key in my_words_dict:
        # 4.1. We create the String
        my_str = key + "\t" + "(" + str(my_words_dict[key]) + ")" + "\n"

        # 4.2. We write it to the file
...
if __name__ == '__main__':
    ...
    if (local_False_hadoop_True == False):
        ...
        my_output_stream = "../../../my_results/Example1_Word_Count/2_MapReduce_Approach/1_my_map_simulation/" + file_name[:-4] + ".txt"
