...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        # 3.1. We get the words from the line
        words = process_line(line)

        # 3.2. We insert each word in the dictionary
        for w in words:
            # 3.2.1. We get the first letter
            letter = w[0]

            # 3.2.2. We check if the letter has appeared in the dictionary before. If not, we enter the letter
            if (letter not in my_letter_words_dict):
                my_letter_words_dict[letter] = (0, 0)

            # 3.2.3. Update the dictionary with the new word
            my_letter_words_dict[letter] = (
            my_letter_words_dict[letter][0] + 1, my_letter_words_dict[letter][1] + len(w))

    # 4. We write the content from my_words_dict into the result file
    for key in my_letter_words_dict:
        # 4.1. We get the value from the dictionary
        (num_words, total_length_words) = my_letter_words_dict[key]

        # 4.2. We create the String
        my_str = key + "\t" + "(" + str(num_words) + ", " + str(total_length_words) + ")" + "\n"

        # 4.3. We write it to the file
...
if __name__ == '__main__':
    ...
    if (local_False_hadoop_True == False):
        ...
        my_output_stream = "../../../my_results/Example2_Word_Count_and_Average/2_MapReduce_Approach/1_my_map_simulation/" + file_name[:-4] + ".txt"
