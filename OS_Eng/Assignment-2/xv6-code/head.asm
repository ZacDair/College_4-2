...
main(int argc, char *argv[])
{
  ...
  51:	e8 4a 00 00 00       	call   a0 <head>
    close(fd);
...
main(int argc, char *argv[])
{
  ...
  7f:	52                   	push   %edx
  80:	52                   	push   %edx
  81:	68 31 08 00 00       	push   $0x831
  86:	6a 00                	push   $0x0
  88:	e8 13 00 00 00       	call   a0 <head>
    exit();
...
head(int fd, char *name)
{
  aa:	81 ec 1c 02 00 00    	sub    $0x21c,%esp
  int i, n, j;
  int l;
  l = j = 0;
  char line [512];
  ...
  b8:	68 60 0b 00 00       	push   $0xb60
  bd:	ff 75 08             	pushl  0x8(%ebp)
  c0:	e8 f5 02 00 00       	call   3ba <read>
  c5:	83 c4 10             	add    $0x10,%esp
  c8:	83 f8 00             	cmp    $0x0,%eax
  cb:	7e 6c                	jle    139 <head+0x99>
    for(i=0; i<n && l < 10; i++){
  cd:	83 ff 09             	cmp    $0x9,%edi
  d0:	7f de                	jg     b0 <head+0x10>
  d2:	31 db                	xor    %ebx,%ebx
  d4:	eb 20                	jmp    f6 <head+0x56>
  ...
  d9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
       ...
       line[j] = buf[i];
  e0:	88 94 35 e8 fd ff ff 	mov    %dl,-0x218(%ebp,%esi,1)
       j++;
  e7:	83 c6 01             	add    $0x1,%esi
  int i, n, j;
  int l;
  l = j = 0;
  char line [512];
  ...
  ed:	39 d8                	cmp    %ebx,%eax
  ef:	7e bf                	jle    b0 <head+0x10>
  f1:	83 ff 09             	cmp    $0x9,%edi
  f4:	7f ba                	jg     b0 <head+0x10>
      if(buf[i] == '\n'){
  f6:	0f b6 93 60 0b 00 00 	movzbl 0xb60(%ebx),%edx
  fd:	80 fa 0a             	cmp    $0xa,%dl
 100:	75 de                	jne    e0 <head+0x40>
 102:	89 85 e4 fd ff ff    	mov    %eax,-0x21c(%ebp)
        line[j] = buf[i];
        line[j+1] = '\0';
        printf(1, line);
 108:	8d 85 e8 fd ff ff    	lea    -0x218(%ebp),%eax
 10e:	83 ec 08             	sub    $0x8,%esp
  ...
  while((n = read(fd, buf, sizeof(buf))) > 0){
    for(i=0; i<n && l < 10; i++){
      if(buf[i] == '\n'){
        line[j] = buf[i];
 111:	c6 84 35 e8 fd ff ff 	movb   $0xa,-0x218(%ebp,%esi,1)
 118:	0a 
        line[j+1] = '\0';
 119:	c6 84 35 e9 fd ff ff 	movb   $0x0,-0x217(%ebp,%esi,1)
 120:	00 
        printf(1, line);
        j = 0;
        l++;
 121:	83 c7 01             	add    $0x1,%edi
  while((n = read(fd, buf, sizeof(buf))) > 0){
    for(i=0; i<n && l < 10; i++){
      if(buf[i] == '\n'){
        line[j] = buf[i];
 ...
 131:	8b 85 e4 fd ff ff    	mov    -0x21c(%ebp),%eax
 137:	eb b1                	jmp    ea <head+0x4a>
       j++;
      }
 ...
 146:	68 20 08 00 00       	push   $0x820
 14b:	6a 01                	push   $0x1
 14d:	e8 ae 03 00 00       	call   500 <printf>
    exit();
 152:	e8 4b 02 00 00       	call   3a2 <exit>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 49f:	f7 f6                	div    %esi
 4a1:	0f b6 92 50 08 00 00 	movzbl 0x850(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 618:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 61b:	b8 48 08 00 00       	mov    $0x848,%eax
